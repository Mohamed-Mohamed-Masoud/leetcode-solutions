...
Approach
1. Use UNION ALL to merge both requester_id and accepter_id into a single, unified list of IDs.
2. Group this combined list by id.
3. Use COUNT(*) to calculate the total number of friends for each user.
4. Sort the results in descending order based on the count (ORDER BY num DESC).
